...
| **4. Структура состояния**      | Единое, **нормализованное** состояние в виде plain JS-объекта. Часто неизменяемое (с помощью Immer).                                            | **Денормализованное**, может состоять из множества взаимосвязанных **"store"**-классов.                                                            |
...
В проекте расставлены `console.log()` для отслежавание потоков store, для того чтобы увидеть `console.log()`, требуется открыть вкладку в режиме инкогнито.

1.  При создании задачи. Код начнет выполняться сверху вниз, выполняя каждую функцию, не откладывая ее, из-за чего чтение станет "явным" и в console мы увидим приходящие console по порядку:
    ```javascript
    console.log('1 до строчки выполнения action:', tasksByStatus);
    console.log('2 после строчки выполнения action:', tasksByStatus);
    ```
    Тут стоит обратить внимание: Redux не изменит состояние сразу, т.к. работает с копией объекта и имеет **иммутабельное** состояние. Состояние обновится только на следующий рендер компонента, когда придет новый объект.
...
В коде по ключевому слову "COMMENT" можно найти закомментированный код, который покажет скорость выполнения создания 1000 задач.
...
В коде по ключевому слову "COMMENT" можно найти закомментированный код (checkMemory), который покажет, сколько требуется памяти для создания задач.

- redux - Было занято памяти: 21.21 MB
- mobx - Было занято памяти: 64.71 MB
  Итог: Redux - примерно в 3 раза меньше занимает памяти!!!

# "@next/bundle-analyzer"

"@next/bundle-analyzer" - это официальный пакет от Next.js для анализа размера сборки вашего приложения.

`npm run build:analyze` - запускает сборку приложения и открывает 2 вкладки серверного и клиентского bundle.

В поиске "Filter to initial chunks:" можно выбрать путь файла, чтобы более детально посмотреть bundle.
Это позволит находить большие bundle и разбивать их.
Также можно переносить bundle с серверного и клиентского, используя `const Component = dynamic(() => import('...'));`

# Redux DevTools

Redux DevTools — это инструмент разработчика для отладки состояния приложения в реальном времени.
У Redux DevTools есть 2 способа использования: установить пакет в проект или скачать расширение в Chrome.
Лично я использовал 2-й способ, что намного удобнее и быстрее.

### Вкладка "Inspector"

    Основная панель для мониторинга действий и состояния. На ней можно видеть все действия, что происходят в Redux.

### Вкладка "State"

    Показывает состояние всего Redux в виде object, где можно увидеть хранимое состояние.

### Вкладка "Action"

    Там видно, какой Action срабатывает и что в него передано.
...
### Вкладка "log Monitor"
    ...
    Показывает, какие action выполняются и какое состояние после action становится.
...
### Вкладка "Dispatcher"
    ...
    Позволяет вручную отправить action из расширения.
...
### Robots.txt (https://developers.google.com/search/docs/crawling-indexing/robots/intro?hl=ru)
...
(Если поисковой робот от Google не провалится по путям, это не значит, что поисковый злоумышленник поступит также.)
...
#### Разбор некоторых тегов более детально
...
На сайте https://developers.google.com/search/docs/crawling-indexing/special-tags?hl=ru есть возможность просмотра, какие мета-теги не поддерживаются или поддерживаются и для чего они нужны более подробно.
